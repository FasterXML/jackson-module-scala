...
class CaseClassSerializerTest extends SerializerTest {
  ...
  it should "serialize a case class with JsonTypeInfo" in {
    serialize(JsonTypeInfoCaseClass(1)) should equal( """{"class":"tools.jackson.module.scala.ser.JsonTypeInfoCaseClass","intValue":1}""")
  }

  it should "serialize a case class containing a case class with JsonTypeInfo" in {
    serialize(CaseClassContainingJsonTypeInfoCaseClass(JsonTypeInfoCaseClass(1))) should equal( """{"c":{"class":"tools.jackson.module.scala.ser.JsonTypeInfoCaseClass","intValue":1}}""")
  }
